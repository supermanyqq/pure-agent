...
| `/model` | 显示当前模型与使用方式。 |
| `/model <model-id>` | 切换后续请求的模型，保留已有消息历史。 |
| `/effort` | 显示当前思考深度与 DeepSeek 映射。 |
| `/effort off|low|medium|high` | 切换后续请求的思考深度。 |

命令绝不能进入聊天消息历史，也不能在流式响应或工具执行期间执行。输入以 `/` 开头时，输入栏上方显示可过滤的命令提示；状态栏始终显示 `model · effort · 状态`，使当前运行设置可见。`Ctrl+C` 仅中止当前轮次，终端的常规退出行为交给 Ink。
...
`config set api-key` 默认从不回显的终端输入读取；非交互环境只能使用 `--stdin`。命令不接受 API Key 位置参数，避免密钥写入 shell history。写入过程创建目录权限 `0700`、临时文件权限 `0600`，随后以原子重命名替换目标文件；`config show` 只输出 `sk-…` 形式的脱敏值。环境变量优先级不变，且不得被写入磁盘。
...
packages/cli/src/
  ...
  hooks/useAgent.ts         消息历史、AgentLoop、运行时设置和命令分发
...
- 缺少或空 API Key：命令返回可操作错误，不创建无效配置文件。
- 未知命令、缺少参数、非法 effort：显示用法并保持会话设置不变。
...
2. CLI 纯函数测试验证命令解析、帮助文案、模型/effort 变更及 DeepSeek 映射。
